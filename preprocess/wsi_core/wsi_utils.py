...
def DrawMapFromCoords(canvas, wsi_object, coords, patch_size, vis_level, indices=None, verbose=1, draw_grid=True):
    ...
    wsi_otsu = cal_wsi_otsu(
        wsi_object.wsi.read_region((0,0), vis_level, wsi_object.wsi.level_dimensions[vis_level]).convert('RGB')
    )
    tr_list = []
    for idx in range(total):
        if verbose > 0:
        ...
        # add Quality Control here, and save the tissue-ratio of each patch in a dataframe
        tr = cal_patch_tissue_ratio(filter_grays(patch), wsi_otsu)
        tr_list.append([coord, tr])
    ...
    return Image.fromarray(canvas), tr_list
...
def StitchCoords(hdf5_file_path, wsi_object, downscale=16, draw_grid=False, bg_color=(0,0,0), alpha=-1):
    ...
    heatmap = np.array(heatmap)
    heatmap, tissueratio = DrawMapFromCoords(heatmap, wsi_object, coords, patch_size, vis_level, indices=None, draw_grid=draw_grid)
    
    file.close()
    return heatmap, tissueratio
...
def blur_image(image_arr):
    img = cv2.cvtColor(image_arr, cv2.COLOR_RGB2HSV)
    img = cv2.medianBlur(img[:,:,1], 7)
    return img

def cal_wsi_otsu(wsi):
    wsi = np.array(wsi)
    wsi = blur_image(wsi)
    thres, _ = cv2.threshold(wsi, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    return thres

def cal_patch_tissue_ratio(image_arr, thres=-1):
    if image_arr.max() <= 60:
        # consider the patch as a black tile
        return -1
    image_arr = blur_image(image_arr)
    fg = np.sum(image_arr > thres*0.6)
    tissue_ratio = fg / image_arr.size
    return tissue_ratio

def filter_grays(image_arr, tolerance=10):
    # input image_arr must be RGB 
    rgb = image_arr.astype(np.int16)
