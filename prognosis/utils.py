...
def collate_MIL_survival_sig(batch):
    img = torch.cat([item[0] for item in batch], dim=0)
    vmisc_feature = torch.cat([item[1] for item in batch], dim=0).type(torch.FloatTensor)

    label = torch.LongTensor([item[2] for item in batch])
    event_time = np.array([item[3] for item in batch])
    c = torch.FloatTensor([item[4] for item in batch])
    case_id = np.array([item[5] for item in batch])
    return [img, vmisc_feature, label, event_time, c, case_id]

def collate_MIL_survival_sig_attn(batch):
    img = torch.cat([item[0] for item in batch], dim=0)
    vmisc_feature = torch.cat([item[1] for item in batch], dim=0).type(torch.FloatTensor)
    plex_feature = torch.cat([item[2] for item in batch], dim=0).type(torch.FloatTensor)

    label = torch.LongTensor([item[3] for item in batch])
    event_time = np.array([item[4] for item in batch])
    c = torch.FloatTensor([item[5] for item in batch])
    case_id = np.array([item[6] for item in batch])
    return [img, vmisc_feature, plex_feature,  label, event_time, c, case_id]

def collate_MIL_survival_sig_attn_coord(batch):
    img = torch.cat([item[0] for item in batch], dim=0)
    vmisc_feature = torch.cat([item[1] for item in batch], dim=0).type(torch.FloatTensor)
    plex_feature = torch.cat([item[2] for item in batch], dim=0).type(torch.FloatTensor)
    coords = np.vstack([item[3] for item in batch])
    ...
    case_id = np.array([item[7] for item in batch])
    return [img, vmisc_feature, plex_feature, coords, label, event_time, c, case_id]
...
def get_split_loader(split_dataset, training=False, testing=False, weighted=False, mode='coattn', batch_size=1):
    ...
    elif mode == 'coattn_attn':
        collate = collate_MIL_survival_sig_attn
    elif mode == 'coattn_attn_coord':
        collate = collate_MIL_survival_sig_attn_coord
    elif mode == 'cluster':
