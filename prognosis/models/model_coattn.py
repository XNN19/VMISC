...
class MCAT_Surv(nn.Module):
    def __init__(self, fusion='concat', omic_sizes=[100, 200, 300, 400, 500, 600], n_classes=4,
        ...
        self.size_dict_WSI = {"small": [2560, 256, 256], "big": [2560, 512, 384]}
        ...
        fc = [nn.Linear(512, size[1]), nn.ReLU()]
        fc.append(nn.Dropout(0.25))
        self.vmisc_net = nn.Sequential(*fc)
    ...
    def forward(self, **kwargs):
        ...
        x_vmisc = kwargs['x_vmisc']
        if x_vmisc is None:
            raise KeyError("MCAT_Surv.forward expects x_vmisc.")
        
        if x_path.dim() == 3 and x_path.size(0) == 1:
            x_path = x_path.squeeze(0)
        if x_vmisc.dim() == 3 and x_vmisc.size(0) == 1:
            x_vmisc = x_vmisc.squeeze(0)

        h_path_bag = self.wsi_net(x_path).unsqueeze(1)  ### path embeddings are fed through a FC layer
        h_vmisc_bag = self.vmisc_net(x_vmisc).unsqueeze(1)

        # Coattn
        h_path_coattn, A_coattn = self.coattn(h_vmisc_bag, h_path_bag, h_path_bag)
        ...
        ### Omics
        h_omic_trans = self.path_transformer(h_vmisc_bag)
